...
It is more flexible and versatile than the graphs produced by the base package, and once you get a grip of the syntax and function arguments, it becomes easy to create beautiful and elaborate visualizations.</br>
...
The remaining elements are "optional", that is, they are set to a default. This means we are not required to define them when we plot, though typically we would want to adjust them, to make our graphs better fit our needs. 

In this presentation I will focus on the first three elements, and the most commonly used geoms.
...
We focused on many examples with scatterplots (code>geom_point()</code>), but the logic of function arguemnts and layers is aplicable to the other geom types.
...
To better demostrate the line geom, We will move to the <code>gapminder</code> data which contains information on life expectancy of countries at different points of time
...
I created a new dataframe via <code>dplyr>/code> functions by grouping continenet and year, and adding a new variable representing the average life expectency.
...
ggplot(yearContinent, aes(year, AverageLifeExp, color = continent)) + 
  geom_smooth(color = "#000000", linetype = "dashed") +
  geom_line() +
